...
{
  flake.aspects.dguibert-custom-profile.nixos.home-manager.users.dguibert.imports = [
  ...
  flake.aspects.dguibert-custom-profile.homeManager =
    ...
    in
    {
      options.withCustomProfile.enable = (lib.mkEnableOption "Enable custom profile") // {
        default = false;
      };
      ...
      config = lib.mkIf config.withCustomProfile.enable {
        ...
        programs.bash.bashrcExtra = # (homes.withoutX11 args).programs.bash.initExtra +
          ...
          '';
        home.activation.setNixVariables =
          lib.hm.dag.entryBefore [ "writeBoundary" "checkLinkTargets" "checkFilesChanges" ]
            ''
              set -x
              export NIX_STATE_DIR=${config.home.sessionVariables.NIX_STATE_DIR}
              export NIX_PROFILE=${config.home.sessionVariables.NIX_PROFILE}
