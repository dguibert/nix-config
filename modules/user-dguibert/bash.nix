...
{
  flake.aspects.dguibert-bash.nixos.home-manager.users.dguibert.imports = [
    config.flake.modules.homeManager.dguibert-bash
  ];
  flake.aspects.dguibert-bash.homeManager =
    { pkgs, config, ... }:
    {
      options.withBash.enable = (lib.mkEnableOption "Enable bash config") // {
        default = true;
      };
      options.withBash.history-merge = (lib.mkEnableOption "Enable bash history merging") // {
        default = true;
      };
